...
export default function NewPostScreen({ session, onCancel, onPosted }) {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      console.log('Error creando publicación:', err);
  ...
  return (
    <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: '#020617', zIndex: 1000 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingTop: 50, paddingBottom: 16 }}>
        ...
        <Text style={{ color: '#ffffff', fontSize: 16, fontWeight: '800' }}>Nueva publicación</Text>
        <View style={{ width: 34 }} />
      </View>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 20 }}>
          {images.map((url, index) => (
            <View key={url} style={{ marginRight: 10, position: 'relative' }}>
              <Image source={{ uri: url }} style={{ width: 90, height: 90, borderRadius: 14, backgroundColor: '#1e293b' }} />
              <TouchableOpacity
          ...
          {images.length < MAX_IMAGES && (
            <TouchableOpacity
              ...
              style={{ width: 90, height: 90, borderRadius: 14, borderWidth: 1, borderStyle: 'dashed', borderColor: '#334155', alignItems: 'center', justifyContent: 'center' }}
            >
              {uploadingImage ? (
      ...
      <View style={{ paddingHorizontal: 20, paddingBottom: 24 }}>
        ...
        >
          {posting ? <ActivityIndicator color="#000000" /> : <Text style={{ color: '#000000', fontWeight: '800', fontSize: 15 }}>Publicar</Text>}
        </TouchableOpacity>
      </View>
    </View>
  );
